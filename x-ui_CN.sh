...
update_dev() {
    ...
    confirm "此操作将更新 x-ui 到最新 DEV 提交（滚动 'dev-latest' 构建，非稳定版）。数据将保留。是否继续？" "y"
...
reset_user() {
    ...
    echo -e "${green} 请使用新的登录用户名和密码访问 X-UI 面板。请务必牢记！${plain}"
...
check_config() {
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        read -rp "是否立即为 IP 生成 SSL 证书？[y/N]：" gen_ssl
        ...
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                echo -e "${yellow}您可以通过主菜单选项 20（SSL 证书管理）重试。${plain}"
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ "${running_in_docker}" == "true" ]]; then
            ...
            LOGI "在 Docker 中，面板是容器的主进程。重启容器即可恢复运行："
...
restart_xray() {
    ...
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        if signal_xui USR1; then
            ...
            LOGI "xray-core 重启信号已成功发送，请查看日志信息确认 xray 是否重启成功"
...
enable() {
    ...
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        LOGI "自启动由 Docker 重启策略控制（如 docker-compose.yml 中的 'restart: unless-stopped'）。"
...
disable() {
    ...
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        LOGI "在宿主机上为容器设置 'restart: no' 以禁用自启动。"
...
bbr_menu() {
    if [[ ! -f /root/tcp.sh ]]; then
        echo -e "${yellow}正在下载 BBR 管理脚本...${plain}"
        wget -N --no-check-certificate "https://raw.githubusercontent.com/chiakge/Linux-NetSpeed/master/tcp.sh" -O /root/tcp.sh && chmod +x /root/tcp.sh
    fi
    bash /root/tcp.sh
}
...
show_enable_status() {
    ...
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        echo -e "自启动状态：${green}由 Docker 管理${plain}"
    ...
    if [[ $? == 0 ]]; then
        ...
        echo -e "自启动状态：${green}已启用${plain}"

    else

        echo -e "自启动状态：${red}已禁用${plain}"
...
run_geo_update() {
    ...
    if [[ $? -ne 0 ]]; then
        ...
        echo -e "${red}部分 ${name} 更新失败。请检查上述错误。${plain}"
...
ssl_cert_issue_main() {
    ...
    echo -e "${green}\t6.${plain} 获取 IP 地址 SSL（6 天证书，自动续期）"
    ...
    case "$choice" in
            ...
            if [ -z "$domains" ]; then
                ...
                if echo "$domains" | grep -qw "$domain"; then
                    ...
                    if [[ "${existing_cert}" == "/root/cert/${domain}/"* ]]; then
                        ...
                        LOGI "已清除引用 ${domain} 的面板证书路径；正在重启面板。"
        ...
        3)
            ...
            if [ -z "$domains" ]; then
                ...
                read -rp "请从列表中选择要续期 SSL 证书的域名：" domain
        ...
        4)
            ...
            echo -e "${green}\t2.${plain} 输入自定义证书文件路径（如 certbot、/etc/letsencrypt/...）"
        ...
        5)
            ...
            echo -e "此操作将使用短效配置为您的服务器 IP 获取证书。"
...
ssl_cert_issue_for_ip() {
    ...
    LOGI "使用 Let's Encrypt 短效配置（约 6 天有效期，自动续期）"
    ...
    read -rp "ACME HTTP-01 监听器使用的端口（默认 80）：" WebPort
    ...
    if [[ "${WebPort}" -ne 80 ]]; then
        ...
        LOGI "提示：Let's Encrypt 仍会访问端口 80；请将外部端口 80 转发到 ${WebPort} 以进行验证。"
    ...
    while true; do
        ...
        if is_port_in_use "${WebPort}"; then
            ...
            LOGI "端口 ${WebPort} 正在使用中。"



            local alt_port=""

            read -rp "请输入 acme.sh standalone 监听器的其他端口（留空则中止）：" alt_port
            ...
            LOGI "端口 ${WebPort} 空闲，可以进行 Standalone 验证。"
    ...
    if [ $? -ne 0 ]; then
        ...
        LOGE "请确保端口 ${WebPort} 已开放，且服务器可从互联网访问"
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        ...
        case "$choice" in
            ...
            2)
                ...
                LOGD "建议将 x-ui restart 放在末尾，这样其他服务失败时不会引发错误"
...
ssl_cert_issue_CF() {
    ...
    LOGI "1. Cloudflare API Token（推荐，范围限定为 Zone:DNS:Edit）或 Global API Key + 注册邮箱。"

    LOGI "2. 域名。"

    LOGI "3. 证书签发后，系统将提示您是否为面板设置证书（可选）。"

    LOGI "4. 脚本还支持安装后 SSL 证书的自动续期。"



    confirm "确认以上信息并继续？[y/n]" "y"
...
iplimit_main() {
    ...
    case "$choice" in
        ...
        1)
            ...
            confirm "确认安装 Fail2ban 和 IP 限制？" "y"
        ...
        3)
            ...
            confirm "确认解封 IP 限制 jail 中的所有 IP？" "y"
...
setup_fail2ban_iplimit() {
    ...
    if [[ -n "${XUI_ENABLE_FAIL2BAN+x}" && "${XUI_ENABLE_FAIL2BAN}" != "true" ]]; then
        ...
        echo -e "${yellow}XUI_ENABLE_FAIL2BAN=${XUI_ENABLE_FAIL2BAN}，跳过 Fail2ban 设置。${plain}\n"
            ...
            fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol)
                ...
                if [[ "${release}" != "fedora" ]] && ! dnf repolist enabled 2> /dev/null | grep -qiw epel; then
                    ...
                    dnf install -y epel-release \
                        ...
                        || echo -e "${yellow}无法启用 EPEL 仓库；在此发行版上 fail2ban 仅可从 EPEL 获取。${plain}"
...
SSH_port_forwarding() {
    ...
    if [[ -n "$existing_cert" && -n "$existing_key" ]]; then
        ...
        echo -e "${green}面板已使用 SSL 保护。${plain}"
...
postgresql_status() {
    ...
    if command -v ss > /dev/null 2>&1; then
        ...
        if [[ -n "$listening" ]]; then
            ...
            echo -e "${green}PostgreSQL 正在监听端口 5432：${plain}"

            echo "$listening"

        else

            echo -e "${red}端口 5432 上无监听 - 数据库未运行。${plain}"
...
pg_require_installed() {
    ...
    if ! postgresql_installed; then
        ...
        LOGE "PostgreSQL 未安装。请先使用此菜单中的选项 1（安装 PostgreSQL）。"
...
purge_postgresql() {
    ...
    echo -e "${yellow}此面板正在使用 PostgreSQL。${plain}"

    echo -e "${red}警告：${plain} 清除操作将删除 PostgreSQL 服务器及其所有数据库，"

    echo -e "包括其他应用程序使用的数据库。此操作不可撤销。"

    confirm "同时清除 PostgreSQL 并删除其所有数据？" "n"

    if [[ $? != 0 ]]; then

        LOGI "PostgreSQL 保持安装状态；其数据未被删除。"
...
pg_upgrade_client() {
    ...
    if [[ -n "$want" && ! "$want" =~ ^[0-9]+$ ]]; then
        ...
        LOGE "无效的 PostgreSQL 主版本号 '${want}'（应为数字，如 17）。"
    ...
    if [[ -n "$have" ]]; then
        ...
        LOGI "已安装的 PostgreSQL 客户端工具版本为 ${have}；需要版本 ${want} 或更新。"

    fi

    if [[ "${running_in_docker}" == "true" ]]; then

        LOGI "注意：容器内安装的软件包在容器重建时会丢失。"
    ...
    case "${release}" in
        ...
        fedora | amzn | virtuozzo | rhel | almalinux | rocky | ol | centos)
            ...
            if [[ -z "$want" ]]; then
                ...
                LOGI "postgresql${want} 不在已启用的仓库中；正在添加官方 PostgreSQL yum 仓库..."
        ...
        *)
            ...
            LOGE "不支持的操作系统 '${release}'；请手动安装 PostgreSQL 客户端工具。"
    ...
    if [[ -z "$have" ]]; then
        ...
        LOGE "安装后 pg_dump/pg_restore 仍然不可用。"
    ...
    if [[ -n "$want" && "$have" -lt "$want" ]]; then
        ...
        LOGE "安装后 PostgreSQL 客户端工具版本为 ${have}，但需要 ${want} 或更新；请手动安装。"
...
pg_install_server_action() {
    ...
    if postgresql_installed; then
        ...
        LOGI "此系统上似乎已安装 PostgreSQL。"

        confirm "仍然运行设置（确保 xui 数据库/用户存在）？" "n" || return 0
...
migrate_to_postgres() {
    ...
    echo -e "${yellow}此操作将当前 SQLite 数据复制到 PostgreSQL 数据库，${plain}"

    echo -e "${yellow}然后将面板切换到 PostgreSQL 并重启。${plain}"

    echo -e "${red}目标数据库中任何现有的面板表将被清除并覆盖。${plain}"
    ...
    if [[ -n "$PG_LAST_DSN" ]]; then
        ...
        echo -e "此次会话中已创建 PostgreSQL 数据库："
    ...
    if [[ -z "$dsn" ]]; then
        ...
        echo -e "${green}\t2.${plain} 使用现有 PostgreSQL 服务器（输入 DSN）"
        ...
        if [[ "$pg_mode" == "2" ]]; then
            ...
            LOGI "正在本地安装 PostgreSQL（可能需要一些时间）..."

            dsn=$(pg_install_local)

            if [[ $? -ne 0 || -z "$dsn" ]]; then

                LOGE "PostgreSQL 安装失败。终止迁移。"
    ...
    if check_status; then
        ...
        LOGE "面板未能启动。请检查日志（主菜单选项 17）。您的 SQLite 数据保持完整。"
...
postgresql_menu() {
    ...
    echo -e "${green}\t1.${plain} ${green}安装${plain} PostgreSQL（服务器 + 客户端 + xui 数据库）"
    ...
    case "$choice" in
        ...
        9)
            ...
            read -rp "所需的 PostgreSQL 主版本号（留空为任意）：" pg_client_ver
...
migrate_db() {
    ...
    if [[ ! -x "$bin" ]]; then
        ...
        LOGE "未找到 x-ui 二进制文件 ${bin}。面板是否已安装？"
    ...
    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        ...
        LOGE "此 x-ui 构建版本尚不支持 .db <-> .dump 转换。"

        LOGE "请先更新面板（x-ui update）到包含 'migrate-db --dump/--restore' 的版本。"
    ...
    if [[ ! -f "$input" ]]; then
        ...
        echo -e "用法：${green}x-ui migrateDB [file.db|file.dump] [output]${plain}"
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            ...
            LOGE "拒绝在 x-ui 运行时还原到活动数据库 (${default_db})。"

            LOGE "请先停止面板（x-ui stop）或选择其他输出路径。"
...
migrate_db_prompt() {
    ...
    echo -e "在 SQLite ${green}.db${plain} 和可移植的 ${green}.dump${plain} 之间转换（自动检测方向）。"

    read -rp "输入文件 [${default_db}]：" input

    input="${input:-$default_db}"

    read -rp "输出文件（留空则自动命名为输入文件旁的同名文件）：" output
...
│  ${blue}x-ui update-dev${plain}            - 更新到 Dev 通道（最新）   │
...
│  ${green}16.${plain} 日志管理                                  │
│────────────────────────────────────────────────│
│  ${green}17.${plain} 启用自启动                                │
│  ${green}18.${plain} 禁用自启动                                │
│────────────────────────────────────────────────│
